    ...
    if (!isset($configures[$file])) {
        ...
        $default = array(
            ...
            'app_autoload' => array(
                ...
                '^(DBQuery|Pagebreak|Cookie|PHPMailer|SMTP|Cache|Logger|Validate|Upload|QQWry|Image|Httplib)$' => UPF_PATH . '/lib/$1.php',
            ),
            // app route rules
            'app_routes' => array(),
        );
    ...
    public function dispatch($handler) {
        ...
        try {
            ...
            if ($handler && class_exists($handler)) {
                ...
                $handle = new $handler();
                $method = apply_filters('apprun_method', strtolower($_SERVER['REQUEST_METHOD']));
                // cli mode
                if (strlen($method) == 0) {
                    $method = 'run';
                }
                // XHR (must come first), iPad, mobile catch all
                elseif (is_xhr_request() && method_exists($handler, $method . '_xhr')) {
                    // no cache
                ...
                // ipad
                elseif (is_ipad_request() && method_exists($handler, $method . '_ipad')) {
                    $method.= '_ipad';
                }
                // mobile
                elseif (is_mobile_request() && method_exists($handler, $method . '_mobile')) {
                    $method.= '_mobile';
                }
                // arguments
                ...
                if (method_exists($handler, $method)) {
                    ...
                    // filters
                    $prev_filter = $method . '_prev'; $next_filter = $method . '_next';
                    // 方法前置事件
                    if (method_exists($handler, $prev_filter)) {
                        call_user_func_array(array(&$handle, $prev_filter), $arguments);
                    }
                    // 执行方法
                    call_user_func_array(array(&$handle, $method), $arguments);
                    // 方法后置事件
                    if (method_exists($handler, $next_filter)) {
                        call_user_func_array(array(&$handle, $next_filter), $arguments);
                    }
                    // 通用后置事件
